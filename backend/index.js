...
mongoose
  .connect('mongodb+srv://divyanshuraj43435_db_user:9FvDgGOUROCOGdoh@smartindia.6uydl5q.mongodb.net/?retryWrites=true&w=majority&appName=smartindia')
  .then(() => {
...
if (!records.length) {
    return res.json({ predictions: [], message: "No traffic data available for prediction" })
}
...
console.log(err)
res.status(503).json({ predictions: [], message: "Prediction service is unavailable" })
}
